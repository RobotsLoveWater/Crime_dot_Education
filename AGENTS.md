...
Python implementations live under `src/mn_sentencing_explorer/`. The historical root modules
(`app.py`, `data.py`, `cache.py`, `account.py`, and peers) are intentionally tiny compatibility
aliases so existing imports, `uv run python cache.py`, `flask --app app`, and `gunicorn app:app`
remain unchanged. In the table below, a historical bare module name refers to its packaged
implementation unless the entry explicitly describes the compatibility alias.
...
| `scripts/inspect_history_key.py` | Small developer utility that prints a representative history→cache key. Replaces the former root `test.py` scratch file. |
...
| `tests/regression/` | Canonical implementations of the two regression guardrails above. The root files are compatibility entry points preserving the historical commands. |
| `scripts/reset-instance.ps1` | Archives `user/` + `classes/` into a timestamped, git-ignored instance archive and recreates clean stores without touching `cache/`; supports `-WhatIf`, confirmation, rollback, and a future `-InstanceRoot`. Its dependency-free fixture is `tests/scripts/reset-instance.Tests.ps1`. |
