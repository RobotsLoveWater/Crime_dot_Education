...
from ..paths import CACHE_DATA_DIR, DATASET_PATH, RAW_CSV_PATH, RAW_PARQUET_PATH
...
DATAFILE_PARQUET = str(RAW_PARQUET_PATH)
DATAFILE_CSV = str(RAW_CSV_PATH)
DATAFILE = DATAFILE_PARQUET if os.path.exists(DATAFILE_PARQUET) else DATAFILE_CSV
DATAPATH = str(CACHE_DATA_DIR) + os.sep
...
def main():
    """Build the runtime base files and optionally warm the disk cache."""
    dataobj = Data()
    dataobj.load(str(DATASET_PATH))

    if input('will you create a raw csv? ').lower() == 'y':
        dataobj.save(str(RAW_CSV_PATH.with_suffix('')))
    ...
    if input('will you create a raw parquet? ').lower() == 'y':
        ...
        if os.path.exists(DATAFILE_CSV):
            ...
            csv_base.save_parquet(str(RAW_PARQUET_PATH.with_suffix('')))
