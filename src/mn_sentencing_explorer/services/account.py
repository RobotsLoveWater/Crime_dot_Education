...
def get_classcode_list() -> list[str]: return os.listdir(USER_DIR)
...
def get_user_list(classcode) -> list[str]:
    ...
    class_directory = USER_DIR / classcode
    ...
    if class_directory.exists():
        ...
        return [name[:-len('.pickle')] for name in os.listdir(class_directory)
...
def find_userids_by_username(username, include_educator=True) -> list[str]:
    ...
    for classcode in sorted(get_classcode_list()):
        ...
        if not (USER_DIR / classcode).is_dir():
            continue
...
def get_user_directory(userid) -> str: return str(USER_DIR / (userid + '.pickle'))
...
def create(username, classcode, password, overwrite=False, classes=None) -> dict:
    ...
    class_directory = USER_DIR / classcode
    if not class_directory.exists(): class_directory.mkdir()  # create directory if needed
